...
public class ccJumpNew : MonoBehaviour
{
    ...
    float lastHight;
    bool climbArea, vaultArea, jumpActive, vaultingAboveNot, isForwards;
    public bool isGrounded;
    public float jumpForce = 300f, movementSpeedClimb = 280000f, movementSpeedClimbMax = 2000000f;
    ...
    void Update()
    {
        if (isForwards == true && GetComponent<ccLocks>().controllerSwitch == false)
        {
            Debug.Log("moke weed erryday");
            GetComponent<Rigidbody>().AddForce(transform.forward * 20000 * 0.0375f);
        ...
        else if (isForwards == true && GetComponent<ccLocks>().controllerSwitch == true)
        {
            Debug.Log("ingest meth");
            GetComponent<Rigidbody>().AddForce(transform.forward * 20000 * 0.0375f);
        }
        ...
        if (GetComponent<ccLocks>().mainSwitch == true && GetComponent<ccLocks>().jumpSwitch == true)
        {
            ...
            if (vaultArea == false && climbArea == false)
            {
                if (Input.GetButtonDown("JumpXbox") && GetComponent<ccLocks>().controllerSwitch == true && isGrounded == true || Input.GetButtonDown("Jump") && GetComponent<ccLocks>().controllerSwitch == false && isGrounded == true)
                {
                    ...
                    GetComponent<Rigidbody>().AddRelativeForce(0, 0, 30, ForceMode.Impulse);
