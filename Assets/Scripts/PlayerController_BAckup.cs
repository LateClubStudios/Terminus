...
public class PlayerController_BAckup : MonoBehaviour {
	...
	void Update () {
		...
		if (Input.GetKeyUp("w") && isGrounded == false)
		{
		...
		if (Input.GetKeyDown("s"))
		{
			...
			b.size = new Vector3(0.41f, 0.835f, 0.39f);
			b.center = new Vector3(0f, 0.42f, 0f);
		...
		if (Input.GetKeyUp("s"))
		{
			...
			//gameObject.gameObject.transform.localScale = new Vector3(4f, 4f, 4f);
		}
    ...
    void FixedUpdate()
    {
        ...
        // bvackwards
        if (walkBackBool == true && sprintBool == false)
        {
            Debug.Log("Player is walking");
            playerSpeed += walkPower;
